...
async def activate_trial_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    trial_hours_str = get_setting('trial_duration_hours', '')
    trial_hours = int(trial_hours_str) if trial_hours_str and trial_hours_str.isdigit() and int(trial_hours_str) > 0 else 0
    if trial_hours > 0:
        key_id = create_initial_vpn_key(internal_user_id, tariff_id, 0, traffic_limit=traffic_limit_bytes, hours=trial_hours)
        logger.info(f'Пробный ключ создан на {trial_hours} часов')
    else:
